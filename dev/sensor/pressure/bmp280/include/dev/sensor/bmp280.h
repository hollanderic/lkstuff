...
typedef enum {
    BMP280_POSR_SKIP,   // This will disable pressure measurement
    BMP280_POSR_X1,
    BMP280_POSR_X2,
    BMP280_POSR_X4,
    BMP280_POSR_X8,
...
typedef enum {
    BMP280_TOSR_SKIP,  // This will disable temperature measurement
    BMP280_TOSR_X1,
    BMP280_TOSR_X2,
    BMP280_TOSR_X4,
    BMP280_TOSR_X8,
...
// initialize device on i2c bus at i2c_addr.  If config is not NULL
// the device will be intialized with parameters stored in confic struct
// Returns NULL if the device could not be initialized or if the device
// has previously been initialized (dont allow re-init of an inited device)
bmp280_dev_t *bmp280_init(int bus, int i2c_addr, bmp280_config_t *config);

// deinit the device and free resources
status_t bmp280_deinit(bmp280_dev_t *dev);

// get device at i2c bus and i2c addr
// returns null if device has not been successfully initialized
bmp280_dev_t *bmp280_get_dev_at_addr(int bus, int i2c_addr);
// get device at index (in list of currently initialized devices)
// returns null if idx >= current number of initialized devices
...
static inline status_t bmp280_write_reg(bmp280_dev_t *dev, uint8_t reg, uint8_t val) {
    uint8_t data[2] = {reg, val};
    return i2c_transmit(dev->bus, dev->i2c_addr, data, 2);
}
